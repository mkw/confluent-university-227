...
public class ShakespeareConverterConsumerProducer {
  ...
  public void createConsumer() throws InterruptedException {
    ...
    prodProps.setProperty(ProducerConfig.BOOTSTRAP_SERVERS_CONFIG, "localhost:9092,localhost:9093");
    ...
    prodProps.setProperty(AbstractKafkaAvroSerDeConfig.SCHEMA_REGISTRY_URL_CONFIG, "http://localhost:28081");

    try (KafkaProducer<Object, Object> producer = new KafkaProducer<>(prodProps)) {

      // Properties for the Consumer
      Properties props = new Properties();
      props.setProperty(ConsumerConfig.BOOTSTRAP_SERVERS_CONFIG, "localhost:9092,localhost:9093");
      ...
      try (KafkaConsumer<String, String> consumer = new KafkaConsumer<>(props)) {
        consumer.subscribe(Collections.singletonList("shakespeare_topic"));
        ...
        while (true) {
          ...
          for (ConsumerRecord<String, String> record : records) {
            ...
            ProducerRecord<Object, Object> avroOtherRecord =
                new ProducerRecord<>("shakespeare_avro_other_topic", shakespeareKey, shakespeareLine);

            producer.send(avroRecord);
            producer.send(avroOtherRecord);
          }
        }
      }
    }
